...
def _client() -> httpx.AsyncClient:
    ...
    return httpx.AsyncClient(
        ...
        headers={"X-Bot-Key": settings.OPSAI_API_KEY},
        timeout=_TIMEOUT,
    )
...
async def opsai_get(path: str, params: dict | None = None) -> dict:
    ...
    async with _client() as client:
        log.info("opsai_get", path=path, params=params)
        try:
            resp = await client.get(path, params=params)
            resp.raise_for_status()
            return resp.json()
        except httpx.HTTPStatusError as exc:
            log.error("opsai_get_http_error", path=path, status=exc.response.status_code)
            return {
                "error": f"OPS-AI returned {exc.response.status_code}",
                "detail": exc.response.text[:500],
            }
        except httpx.TimeoutException:
            log.error("opsai_get_timeout", path=path)
            return {"error": "OPS-AI request timed out after 15s"}
        except Exception as exc:
            log.error("opsai_get_failed", path=path, error=str(exc))
